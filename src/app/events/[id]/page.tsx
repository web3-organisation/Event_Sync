...
import type { EventDetail, SessionSummary, RoomWithSessions } from "@/lib/types";
import {
  formatEventDate,
  formatTimeRange,
  getSessionStatus,
...
export default function EventDetailPage() {
  ...
  const [view, setView] = useState<"rooms" | "timeline">("rooms");
  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--ghost)" }}>
      {/* Header */}
      <header style={{ background: "var(--navy)", padding: "3rem 2rem 2.5rem", position: "relative", overflow: "hidden" }}>
        <div
        ...
        <div style={{ maxWidth: "980px", margin: "0 auto", position: "relative" }}>
          <Link
            ...
            style={{
              ...
              color: "rgba(248,249,255,0.55)", fontSize: "0.85rem",
              textDecoration: "none", marginBottom: "1.5rem",
              transition: "color 0.15s",
            }}
            onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.color = "white")}
            onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.color = "rgba(248,249,255,0.55)")}
          >
          ...
          <h1
            ...
            style={{
              color: "white", fontSize: "clamp(1.6rem, 4vw, 2.6rem)",
          ...
          {event.description && (
            <p
              ...
              style={{ color: "rgba(248,249,255,0.65)", fontSize: "1rem", maxWidth: "600px", lineHeight: 1.6, marginBottom: "1.25rem" }}
            >
              {event.description}
            </p>
          )}

          <div className="animate-slide-up s2" style={{ display: "flex", flexWrap: "wrap", gap: "1.25rem" }}>
            ...
            <HeaderMeta icon={<Users size={15} />}>
              {event.rooms.length} salle{event.rooms.length !== 1 ? "s" : ""} · {event.sessions.length} session{event.sessions.length !== 1 ? "s" : ""}
            </HeaderMeta>
          </div>
        </div>
      </header>
      ...
      {/* Date tabs */}
      {dates.length > 1 && (
        <div style={{ background: "white", borderBottom: "1px solid rgba(26,26,46,0.08)", overflowX: "auto" }}>
          <div style={{ maxWidth: "980px", margin: "0 auto", padding: "0 2rem", display: "flex", gap: "4px" }}>
            {dates.map((d) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    fontSize: "0.88rem", fontFamily: "inherit", fontWeight: active ? 700 : 500,
      ...
      <div style={{ maxWidth: "980px", margin: "0 auto", padding: "1.5rem 2rem 0" }}>
        <div style={{ display: "flex", gap: "6px", background: "white", borderRadius: "10px", padding: "4px", width: "fit-content", boxShadow: "0 1px 6px rgba(26,26,46,0.08)" }}>
          {(["rooms", "timeline"] as const).map((v) => (
            <button
              key={v}
              onClick={() => setView(v)}
              style={{
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span style={{ marginLeft: "auto", color: col.accent, fontSize: "0.78rem", fontWeight: 600 }}>
...
function TimelineView({
  ...
  const COLORS = [
    "var(--violet)", "var(--teal)", "var(--coral)", "var(--sky)", "var(--golden)", "var(--green)",
  ];
    ...
    >
      <div
        ...
        onMouseEnter={(e) => {
          ...
          (e.currentTarget as HTMLElement).style.boxShadow = `0 4px 18px rgba(26,26,46,0.1)`;
        }}
        onMouseLeave={(e) => {
        ...
        <div style={{ flex: 1, padding: "0.9rem 1.1rem", background: "white" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "0.75rem", flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "0.35rem", flexWrap: "wrap" }}>
                <span
                  style={{
                    ...
                    color: "rgba(26,26,46,0.5)", fontSize: "0.78rem", fontWeight: 600,
                  }}
                >
                  <Clock size={12} />
                ...
                >
                  {cfg.dot && (
                    <span
                      ...
                      style={{ width: "5px", height: "5px", borderRadius: "50%", background: cfg.color, display: "inline-block" }}
                    />
                  )}
                  {cfg.label}
                </span>
              ...
              <h3 style={{ color: "var(--navy)", fontWeight: 700, fontSize: "0.97rem", margin: "0 0 0.35rem", lineHeight: 1.35 }}>
                {session.title}
              </h3>

              {session.description && (
                <p style={{ color: "rgba(26,26,46,0.55)", fontSize: "0.82rem", lineHeight: 1.5, margin: "0 0 0.55rem", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                  {session.description}
                </p>
              )}
              ...
              {speakers.length > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                  ...
                  {speakers.map((sp) => (
                    <span key={sp.id} style={{ fontSize: "0.78rem", color: "rgba(26,26,46,0.6)", fontWeight: 500 }}>
                      {sp.fullName}
                    </span>
                  ))}
                </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "6px", flexShrink: 0 }}>
              {session._count.questions > 0 && (
                <span style={{ fontSize: "0.75rem", color: "rgba(26,26,46,0.4)", fontWeight: 600 }}>
...
/* ─── Small helpers ──────────────────────────────────────────────────── */
function HeaderMeta({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", color: "rgba(248,249,255,0.7)", fontSize: "0.88rem" }}>
      {icon}
      {children}
    </span>
  );
